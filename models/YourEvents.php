 ...
 * 
 */
class YourEvents extends Model {
...
//CLASS methods
    ...
    function __construct($user, $pageTitle, $pageHead, $database) {
        parent::__construct('YourEvents', $user->getLoggedInState());
        $this->user = $user;
...
//end METHOD -   set the page heading
    ...
    public function setPanelHead_1() {//set the panel 1 heading
        $this->panelHead_1 = '<h3>Your Events</h3>';
    }
...
//end METHOD - //set the panel 1 content 
    ...
    public function setPanelHead_2() { //set the panel 3 heading
        switch ($this->user->getUserType()) {
            case 'EVENTADMIN': //An Admin is logged in
                $this->panelHead_2 = 'Here are all the events you are currently administrating';
                break;
            case 'USER':  //A USER is logged in 
                $this->panelHead_2 .= 'Here are all the events you are currently attending';
                break;
            default:
    ...
    public function setPanelContent_2() { //set the panel 2 content
        switch ($this->user->getUserType()) {
            case "EVENTADMIN": //An Admin is logged in
                ...
                if (isset($_POST['btnDelete'])) {
                    ...
                    if ($eventTable->archiveEvent($_POST['id'])) {
                        ...
                        $this->panelContent_2 .= "<h4 class='text-success'>Event Successfully Deleted</h4>";
                    } else
                        $this->panelContent_2 .= "<h4 class='text-danger'>Event Not Deleted</h4>";
                }

                $eventAdminTable = new EventAdminTable($this->db);
                if ($rs = $eventAdminTable->getAdministratingEvents($this->user->getUserID())) {

                    $this->panelContent_2 .= HelperHTML::generateAdminTABLE($rs);
                } else {
                    $this->panelContent_2 .= 'Record not found';
                }
                break;
            ...
            case "USER": //A USER is logged in 
                ...
                if ($rs = $userTable->getAttendingEvents($this->user->getUserID())) {
                    if ($rs->num_rows > 0){
                    $this->panelContent_2 .= HelperHTML::generateTABLE($rs);
                    } else {
                        $this->panelContent_2 .= '<p>You are not currently attending any events</p>';
                    }
                } else {
                    $this->panelContent_2 .= 'Record not found';
                }
