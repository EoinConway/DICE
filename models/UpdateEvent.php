 ...
 * Class: UpdateEvent
 * 
 * This class is used to generate text content for the MODIFY/CANCEL EVENT page view.
 * 
...
//CLASS methods
    ...
    function __construct($user, $pageTitle, $pageHead) {
        parent::__construct('ModifyCancelEvent', $user->getLoggedInState());
        $this->user = $user;
...
//end METHOD -   set the page heading
    ...
    public function setPanelHead_1() {//set the panel 1 heading
        $this->panelHead_1 = '<h3>Modify or Cancel an Event</h3>';
    }
...
//end METHOD - //set the panel 1 content        
    ...
    public function setPanelHead_2() { //set the panel 3 heading
        if ($this->loggedin) {
            if ($this->user->getUserType() === 'EVENTADMIN') { //An Admin is logged in
                $this->panelHead_2 = '<h3>Enter event ID</h3>';
            }
        } else {
...
//end METHOD - //set the panel 2 heading
    ...
    public function setPanelContent_2() { //set the panel 2 content
        if ($this->loggedin) {
            if ($this->user->getUserType() === 'EVENTADMIN') { //An Admin is logged in
                $this->panelContent_2 .= file_get_contents('forms/form_updateEventQuery.php');
            }
        } else {
...
//end METHOD - //set the panel 2 content 
    ...
    public function setPanelHead_3() { //set the panel 3 heading
        if ($this->loggedin) {
            $this->panelHead_3 = '<h3>Event details</h3>';
        } else {
...
//end METHOD - //set the panel 3 heading
    ...
    public function setPanelContent_3() { //set the panel 3 content
        echo 'Set panel 3 content';
        if ($this->loggedin) {
            if (isset($this->postArray['btnSelectEvent'])) {
                $event = new EventTable($this->db);
                if ($rs = $event->getPartialEventByID($this->postArray['eventID'])) {
                    $row = $rs->fetch_assoc();
                           echo $row;
                    $this->panelContent_3 .= file_get_contents('forms/form_updateEvent.php');
                }
            } else {
